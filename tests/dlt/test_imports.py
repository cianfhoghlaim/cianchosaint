"""Phase 2.2 sister-repo smoke test — discover every top-level sub-tree of
...
parent change §14 (the cianchosaint sister-repo init), this test reuses the
Phase 0.1 cianfhoghlaim smoke-test pattern at
`cianfhoghlaim/tests/dlt/test_imports.py` and adapts it for the
standalone cianchosaint sister-repo shape.
...
def _run_smoke() -> dict:
    ...
    summary = {
        "schema_version": 1,
        "change_id": "2026-08-24-cianchosaint-init-v1",
        "phase": "2.2",
        "sister_repo": "cianchosaint",
...
def test_dlt_subtree_imports(tmp_path: Path) -> None:
    ...
    report_path = SYNC_REPORTS_DIR / f"dlt-smoke-run-cianchosaint-{timestamp}.json"
    ...
    print(
        f"\n[cianchosaint:dlt:smoke-all] {summary['total_subtrees']} sub-trees "
...
if __name__ == "__main__":
    ...
    raise SystemExit(
        ...
        "(or `mise run cianchosaint:smoke-all` once the per-sister "
