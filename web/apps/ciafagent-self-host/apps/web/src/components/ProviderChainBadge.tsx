...
export function ProviderChainBadge({ rootAgent, jurisdiction }: { rootAgent: "ga_root_agent" | "met_root_agent" | "psni_root_agent"; jurisdiction: "ga" | "met" | "psni" }) {
  ...
  return (
    <div className="fixed bottom-2 right-2 bg-slate-950 border border-emerald-800 rounded-lg px-3 py-1 text-xs text-slate-400">
      <span className="font-mono">{rootAgent}</span>
      <span className="mx-2 text-slate-600">·</span>
      <span className="font-mono">SELF-HOST</span>
      <span className="mx-2 text-slate-600">·</span>
      <span className="text-emerald-400">Tier {providerTier}</span>
