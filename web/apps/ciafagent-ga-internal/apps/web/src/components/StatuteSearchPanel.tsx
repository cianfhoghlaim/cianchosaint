 ...
 */
// Internal StatuteSearchPanel — for internal statute lookups
...
export function StatuteSearchPanel({ jurisdiction, source }: StatuteSearchPanelProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 flex flex-col gap-6">
      <h1 className="text-3xl font-bold text-blue-300">Internal statute search ({source})</h1>
      <form onSubmit={search} className="flex gap-2">
        <input value={query} onChange={(e) => setQuery(e.target.value)} className="flex-1 bg-slate-900 rounded px-3 py-2 text-sm" />
        <button type="submit" className="bg-blue-700 px-4 py-2 rounded text-sm">Search</button>
      </form>
      <div className="flex flex-col gap-2">
        {results.map((r, i) => (
          <a key={i} href={r.url} target="_blank" rel="noreferrer" className="bg-slate-900 border border-blue-900 rounded p-3 text-sm">
            <span className="text-blue-300">{r.title} · {r.year} · No. {r.number}</span>
          </a>
        ))}
      </div>
      <p className="text-xs text-slate-500">Jurisdiction: {jurisdiction} (internal use only)</p>
    </div>
  );
}
