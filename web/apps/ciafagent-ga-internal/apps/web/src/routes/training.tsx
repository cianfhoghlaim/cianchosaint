 ...
 */
// routes/training.tsx — training modules + e-learning
...
function TrainingComponent() {
  const [modules, setModules] = React.useState<Array<{ id: string; title: string; progress: number; duration: string }>>([]);

  React.useEffect(() => {
    fetch("/api/osint/training/modules").then(r => r.json()).then(d => setModules(d.modules || []));
  }, []);
  ...
  return (
    <div className="max-w-4xl mx-auto p-8 flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 gap-4">
        {modules.map((m) => (
          <div key={m.id} className="bg-slate-900 border border-slate-800 rounded p-4">
