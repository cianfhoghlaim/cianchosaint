...
const SESSION_KEY = "cianchosaint.psni-public.convex.session";
export function useConvexSession() {
  const [session, setSession] = React.useState(() => {
    if (typeof window === "undefined") return { userId: null, isAuthenticated: false };
    try { const raw = window.localStorage.getItem(SESSION_KEY); return raw ? JSON.parse(raw) : { userId: null, isAuthenticated: false }; }
