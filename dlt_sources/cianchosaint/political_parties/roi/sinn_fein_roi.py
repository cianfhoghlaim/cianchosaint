...
"""cianchosaint.cianchosaint.dlt.british_isles.political_parties.roi.sinn_fein_roi — Sinn Féin (ROI).

Source: ``https://www.sinnfein.ie/news`` — Sinn Féin (ROI branch) news
room. Note: Sinn Féin also has a separate NI branch source at
``dlt_sources/cianchosaint/political_parties/ni/sinn_fein_ni.py``.

Falls within the cianchosaint OSINT allowlist (entry:
ig_username=sinnfeinireland).

Honours ``USE_LOCAL_SCRAPES=true`` falling back to
``stedding/ingest_queue/roi/sinn_fein/``.
"""
...
SINN_FEIN_ROI_BASE = "https://www.sinnfein.ie/news"
SINN_FEIN_ROI_ELECTORAL_COMMISSION_ID = "nil"


class SinnFeinROIPipeline(PoliticalPartyPipelineBase):
    """Sinn Féin (Republic of Ireland branch) pipeline."""

    PARTY_ID = "sinn-fein-roi"
    PARTY_NAME = "Sinn Féin (Republic of Ireland branch)"
    JURISDICTION = "roi_dail"
    SOURCE_BASE = SINN_FEIN_ROI_BASE
    ELECTORAL_COMMISSION_ID = SINN_FEIN_ROI_ELECTORAL_COMMISSION_ID

    def _iter_press_releases(self) -> Iterator[dict[str, Any]]:
        for page in _crawl_source(
            source_name="sinn_fein_roi.news",
            base_url=SINN_FEIN_ROI_BASE,
            include_paths=["/**"],
            max_pages=50,
            max_depth=3,
        ):
...
@dlt.source(name="sinn_fein_roi")
def sinn_fein_roi_source() -> list:
    """DLT source for Sinn Féin (ROI branch)."""

    @dlt.resource(
        name="sinn_fein_roi_press_releases",
        write_disposition="merge",
        primary_key=["natural_key"],
    )
    def press_releases() -> Iterator[dict[str, Any]]:
        pipeline = SinnFeinROIPipeline()
