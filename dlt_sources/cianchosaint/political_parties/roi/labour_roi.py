# CIANCHOSAINT new-build: per-political-party DLT source for the Irish
# Labour Party.
#
...
"""cianchosaint.cianchosaint.dlt.british_isles.political_parties.roi.labour_roi — Labour (Ireland).

Source: ``https://www.labour.ie/news/` — the Irish Labour Party news
room. Falls within the cianchosaint OSINT allowlist (entry:
ig_username=labourparty_ie).

Honours ``USE_LOCAL_SCRAPES=true`` falling back to
``stedding/ingest_queue/roi/labour/``.
"""
...
LABOUR_ROI_BASE = "https://www.labour.ie/news/"
LABOUR_ROI_ELECTORAL_COMMISSION_ID = "nil"


class LabourROIPipeline(PoliticalPartyPipelineBase):
    """Irish Labour Party pipeline."""

    PARTY_ID = "labour-roi"
    PARTY_NAME = "Irish Labour Party"
    JURISDICTION = "roi_dail"
    SOURCE_BASE = LABOUR_ROI_BASE
    ELECTORAL_COMMISSION_ID = LABOUR_ROI_ELECTORAL_COMMISSION_ID

    def _iter_press_releases(self) -> Iterator[dict[str, Any]]:
        for page in _crawl_source(
            source_name="labour_roi.news",
            base_url=LABOUR_ROI_BASE,
            include_paths=["/**"],
            max_pages=50,
            max_depth=3,
        ):
...
@dlt.source(name="labour_roi")
def labour_roi_source() -> list:
    """DLT source for the Irish Labour Party."""

    @dlt.resource(
        name="labour_roi_press_releases",
        write_disposition="merge",
        primary_key=["natural_key"],
    )
    def press_releases() -> Iterator[dict[str, Any]]:
        pipeline = LabourROIPipeline()
