# CIANCHOSAINT new-build: per-political-party DLT source for Rise
# (Republic of Ireland).
#
...
"""cianchosaint.cianchosaint.dlt.british_isles.political_parties.roi.rise_roi — Rise (ROI).

Source: ``https://www.riseparty.ie/news`` — Rise (Republic of Ireland)
news room. Falls within the cianchosaint OSINT allowlist.

Honours ``USE_LOCAL_SCRAPES=true`` falling back to
``stedding/ingest_queue/roi/rise/``.
"""
...
RISE_ROI_BASE = "https://www.riseparty.ie/news"
RISE_ROI_ELECTORAL_COMMISSION_ID = "nil"


class RiseROIPipeline(PoliticalPartyPipelineBase):
    """Rise (Republic of Ireland) pipeline."""

    PARTY_ID = "rise-roi"
    PARTY_NAME = "Rise (Republic of Ireland)"
    JURISDICTION = "roi_dail"
    SOURCE_BASE = RISE_ROI_BASE
    ELECTORAL_COMMISSION_ID = RISE_ROI_ELECTORAL_COMMISSION_ID

    def _iter_press_releases(self) -> Iterator[dict[str, Any]]:
        for page in _crawl_source(
            source_name="rise_roi.news",
            base_url=RISE_ROI_BASE,
            include_paths=["/**"],
            max_pages=50,
            max_depth=3,
        ):
...
@dlt.source(name="rise_roi")
def rise_roi_source() -> list:
    """DLT source for Rise (Republic of Ireland)."""

    @dlt.resource(
        name="rise_roi_press_releases",
        write_disposition="merge",
        primary_key=["natural_key"],
    )
    def press_releases() -> Iterator[dict[str, Any]]:
        pipeline = RiseROIPipeline()
