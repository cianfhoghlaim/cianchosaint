...
# Scottish National Party (UK HoC scope).
#
...
"""cianchosaint.cianchosaint.dlt.british_isles.political_parties.uk.snp — SNP (UK HoC).

Source: ``https://www.snp.org/news`` — the Scottish National Party
news room. Note: SNP also has a separate Holyrood scope source at
``dlt_sources/cianchosaint/political_parties/scotland/snp_scottish.py``.

Falls within the cianchosaint OSINT allowlist (entry:
ig_username=snp).

Honours ``USE_LOCAL_SCRAPES=true`` falling back to
``stedding/ingest_queue/uk_hoc/snp/``.
"""
...
SNP_BASE = "https://www.snp.org/news"
SNP_ELECTORAL_COMMISSION_ID = "PP-10122"


class SNPWestminsterPipeline(PoliticalPartyPipelineBase):
    """SNP (UK HoC scope) pipeline."""

    PARTY_ID = "snp"
    PARTY_NAME = "Scottish National Party (UK HoC scope)"
    JURISDICTION = "uk_hoc"
    SOURCE_BASE = SNP_BASE
    ELECTORAL_COMMISSION_ID = SNP_ELECTORAL_COMMISSION_ID

    def _iter_press_releases(self) -> Iterator[dict[str, Any]]:
        for page in _crawl_source(
            source_name="snp.news",
            base_url=SNP_BASE,
            include_paths=["/**"],
            max_pages=50,
            max_depth=3,
...
@dlt.source(name="snp")
def snp_source() -> list:
    """DLT source for the SNP (UK HoC scope)."""

    @dlt.resource(
        name="snp_press_releases",
        write_disposition="merge",
        primary_key=["natural_key"],
    )
    def press_releases() -> Iterator[dict[str, Any]]:
        pipeline = SNPWestminsterPipeline()
