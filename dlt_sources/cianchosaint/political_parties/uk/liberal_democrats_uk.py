# CIANCHOSAINT new-build: per-political-party DLT source for the
# Liberal Democrats.
#
# Original: cianfhoghlaim/cianfhoghlaim (per the openspec/changes/
#   cianchosaint-political-party-pipeline-v1/specs/cianchosaint-political-
#   party-pipeline/spec.md, Requirement: The 24 per-party DLT source
#   modules, Scenario: All 24 parties ship from this change).
# Migrated to cianchosaint: 2026-08-23
# Licence: BUSL-1.1 (per LICENSE.md)

"""cianchosaint.cianchosaint.dlt.british_isles.political_parties.uk.liberal_democrats_uk — Lib Dems.

Source: ``https://www.libdems.org.uk/news`` — the Liberal Democrats
news room. Falls within the cianchosaint OSINT allowlist (entry:
ig_username=libdems).

Honours ``USE_LOCAL_SCRAPES=true`` falling back to
``stedding/ingest_queue/uk_hoc/libdems/``.
"""
from __future__ import annotations

from collections.abc import Iterator
from typing import Any

import dlt
import structlog

import dlt_sources

from dlt_sources.cianchosaint.political_parties._base import (
    PoliticalPartyPipelineBase,
)
from dlt_sources.cianchosaint.political_parties import _crawl_source

logger = structlog.get_logger(__name__)


LIBDEMS_UK_BASE = "https://www.libdems.org.uk/news"
LIBDEMS_UK_ELECTORAL_COMMISSION_ID = "PP-10117"


class LiberalDemocratsUKPipeline(PoliticalPartyPipelineBase):
    """Liberal Democrats pipeline."""

    PARTY_ID = "liberal-democrats-uk"
    PARTY_NAME = "Liberal Democrats"
    JURISDICTION = "uk_hoc"
    SOURCE_BASE = LIBDEMS_UK_BASE
    ELECTORAL_COMMISSION_ID = LIBDEMS_UK_ELECTORAL_COMMISSION_ID

    def _iter_press_releases(self) -> Iterator[dict[str, Any]]:
        for page in _crawl_source(
            source_name="libdems_uk.news",
            base_url=LIBDEMS_UK_BASE,
            include_paths=["/**"],
            max_pages=50,
            max_depth=3,
        ):
            page["party_id"] = self.PARTY_ID
            page["party_name"] = self.PARTY_NAME
            page["jurisdiction"] = self.JURISDICTION
            page["electoral_commission_id"] = self.ELECTORAL_COMMISSION_ID
            yield self.party_to_row(page)


@dlt.source(name="liberal_democrats_uk")
def liberal_democrats_uk_source() -> list:
    """DLT source for the Liberal Democrats."""

    @dlt.resource(
        name="liberal_democrats_uk_press_releases",
        write_disposition="merge",
        primary_key=["natural_key"],
    )
    def press_releases() -> Iterator[dict[str, Any]]:
        pipeline = LiberalDemocratsUKPipeline()
        yield from pipeline._iter_press_releases()

    return [press_releases]