...
"""cianchosaint.cianchosaint.dlt.british_isles.political_parties.uk.plaid_cymru — Plaid Cymru (UK HoC).

Source: ``https://www.partyof.wales/news`` — Plaid Cymru (Party of
Wales) news room. Note: Plaid Cymru also has a separate Senedd scope
source at ``dlt_sources/cianchosaint/political_parties/wales/plaid_cymru_senedd.py``.

Falls within the cianchosaint OSINT allowlist (entry:
ig_username=plaid_cymru).

Honours ``USE_LOCAL_SCRAPES=true`` falling back to
``stedding/ingest_queue/uk_hoc/plaid_cymru/``.
"""
...
PLAID_CYMRU_BASE = "https://www.partyof.wales/news"
PLAID_CYMRU_ELECTORAL_COMMISSION_ID = "PP-10115"


class PlaidCymruUKPipeline(PoliticalPartyPipelineBase):
    """Plaid Cymru (UK HoC scope) pipeline."""

    PARTY_ID = "plaid-cymru"
    PARTY_NAME = "Plaid Cymru — Party of Wales (UK HoC scope)"
    JURISDICTION = "uk_hoc"
    SOURCE_BASE = PLAID_CYMRU_BASE
    ELECTORAL_COMMISSION_ID = PLAID_CYMRU_ELECTORAL_COMMISSION_ID

    def _iter_press_releases(self) -> Iterator[dict[str, Any]]:
        for page in _crawl_source(
            source_name="plaid_cymru.news",
            base_url=PLAID_CYMRU_BASE,
            include_paths=["/**"],
            max_pages=50,
            max_depth=3,
        ):
...
@dlt.source(name="plaid_cymru")
def plaid_cymru_source() -> list:
    """DLT source for Plaid Cymru (UK HoC scope)."""

    @dlt.resource(
        name="plaid_cymru_press_releases",
        write_disposition="merge",
        primary_key=["natural_key"],
    )
    def press_releases() -> Iterator[dict[str, Any]]:
        pipeline = PlaidCymruUKPipeline()
