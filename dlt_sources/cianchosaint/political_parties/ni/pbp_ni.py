...
# Before Profit (Northern Ireland).
#
...
"""cianchosaint.cianchosaint.dlt.british_isles.political_parties.ni.pbp_ni — PBP (NI).

Source: ``https://www.pbp.ie/ni/news/`` — People Before Profit (Northern
Ireland) news room. The NI branch shares the all-island website with
``dlt_sources/cianchosaint/political_parties/roi/pbp_solidarity.py``.

Falls within the cianchosaint OSINT allowlist.

Honours ``USE_LOCAL_SCRAPES=true`` falling back to
``stedding/ingest_queue/ni_assembly/pbp/``.
"""
...
PBP_NI_BASE = "https://www.pbp.ie/ni/news/"
PBP_NI_ELECTORAL_COMMISSION_ID = "PP-10128"


class PBPNIPipeline(PoliticalPartyPipelineBase):
    """People Before Profit (Northern Ireland) pipeline."""

    PARTY_ID = "pbp-ni"
    PARTY_NAME = "People Before Profit (Northern Ireland)"
    JURISDICTION = "ni_assembly"
    SOURCE_BASE = PBP_NI_BASE
    ELECTORAL_COMMISSION_ID = PBP_NI_ELECTORAL_COMMISSION_ID

    def _iter_press_releases(self) -> Iterator[dict[str, Any]]:
        for page in _crawl_source(
            source_name="pbp_ni.news",
            base_url=PBP_NI_BASE,
            include_paths=["/**"],
            max_pages=50,
            max_depth=3,
        ):
...
@dlt.source(name="pbp_ni")
def pbp_ni_source() -> list:
    """DLT source for People Before Profit (Northern Ireland)."""

    @dlt.resource(
        name="pbp_ni_press_releases",
        write_disposition="merge",
        primary_key=["natural_key"],
    )
    def press_releases() -> Iterator[dict[str, Any]]:
        pipeline = PBPNIPipeline()
