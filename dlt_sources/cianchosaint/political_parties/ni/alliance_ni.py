...
"""cianchosaint.cianchosaint.dlt.british_isles.political_parties.ni.alliance_ni — Alliance (NI).

Source: ``https://www.allianceparty.org/news`` — Alliance Party of
Northern Ireland news room. Falls within the cianchosaint OSINT
allowlist (entry: ig_username=allianceparty).

Honours ``USE_LOCAL_SCRAPES=true`` falling back to
``stedding/ingest_queue/ni_assembly/alliance/``.
"""
...
ALLIANCE_NI_BASE = "https://www.allianceparty.org/news"
ALLIANCE_NI_ELECTORAL_COMMISSION_ID = "PP-10119"


class AllianceNIPipeline(PoliticalPartyPipelineBase):
    """Alliance Party of Northern Ireland pipeline."""

    PARTY_ID = "alliance-ni"
    PARTY_NAME = "Alliance Party of Northern Ireland"
    JURISDICTION = "ni_assembly"
    SOURCE_BASE = ALLIANCE_NI_BASE
    ELECTORAL_COMMISSION_ID = ALLIANCE_NI_ELECTORAL_COMMISSION_ID

    def _iter_press_releases(self) -> Iterator[dict[str, Any]]:
        for page in _crawl_source(
            source_name="alliance_ni.news",
            base_url=ALLIANCE_NI_BASE,
            include_paths=["/**"],
            max_pages=50,
            max_depth=3,
        ):
...
@dlt.source(name="alliance_ni")
def alliance_ni_source() -> list:
    """DLT source for the Alliance Party of Northern Ireland."""

    @dlt.resource(
        name="alliance_ni_press_releases",
        write_disposition="merge",
        primary_key=["natural_key"],
    )
    def press_releases() -> Iterator[dict[str, Any]]:
        pipeline = AllianceNIPipeline()
