...
"""cianchosaint.cianchosaint.dlt.british_isles.political_parties.ni.tuv_ni — TUV.

Source: ``https://www.tuv.org.uk/news/`` — Traditional Unionist Voice
news room. Falls within the cianchosaint OSINT allowlist (entry:
ig_username=tuvni).

Honours ``USE_LOCAL_SCRAPES=true`` falling back to
``stedding/ingest_queue/ni_assembly/tuv/``.
"""
...
TUV_BASE = "https://www.tuv.org.uk/news/"
TUV_ELECTORAL_COMMISSION_ID = "PP-10127"


class TUVNIPipeline(PoliticalPartyPipelineBase):
    """Traditional Unionist Voice pipeline."""

    PARTY_ID = "tuv-ni"
    PARTY_NAME = "Traditional Unionist Voice"
    JURISDICTION = "ni_assembly"
    SOURCE_BASE = TUV_BASE
    ELECTORAL_COMMISSION_ID = TUV_ELECTORAL_COMMISSION_ID

    def _iter_press_releases(self) -> Iterator[dict[str, Any]]:
        for page in _crawl_source(
            source_name="tuv_ni.news",
            base_url=TUV_BASE,
            include_paths=["/**"],
            max_pages=50,
            max_depth=3,
...
@dlt.source(name="tuv_ni")
def tuv_ni_source() -> list:
    """DLT source for the Traditional Unionist Voice."""

    @dlt.resource(
        name="tuv_ni_press_releases",
        write_disposition="merge",
        primary_key=["natural_key"],
    )
    def press_releases() -> Iterator[dict[str, Any]]:
        pipeline = TUVNIPipeline()
