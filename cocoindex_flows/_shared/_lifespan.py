...
# names have been renamed CIANFHOGHLAIM_* -> CIANCHOSAINT_* per the
...
# Canonical env-var matrix (CIANCHOSAINT_*); the legacy LANCEDB_URI
# env var is honoured as a backwards-compat alias.
try:
    from cianchosaint.observability.env_config import (
        LANCEDB_URL as _CIANCHOSAINT_LANCEDB_URL,
    )
except ImportError:  # pragma: no cover - the env-config module ships in the same wheel
    _CIANCHOSAINT_LANCEDB_URL = "rest://lakehouse-lance-namespace:8182"
...
# `CIANCHOSAINT_LANCEDB_URL=http://127.0.0.1:8182` or set
# `LANCEDB_URI=rest://lakehouse-lance-namespace:8182` in `.env.dev.local`.
LANCEDB_URI = _CIANCHOSAINT_LANCEDB_URL
EMBED_MODEL = os.getenv("CIANCHOSAINT_EMBED_MODEL", "BAAI/bge-m3")
EMBED_DIM = 1024
