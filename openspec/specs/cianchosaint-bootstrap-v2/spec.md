# cianchosaint-bootstrap-v2 Capability

## Purpose

`cianchosaint-bootstrap-v2` is the umbrella capability for the
**wholesale-copy bootstrap** of the cianchosaint repo. It captures
the contract that cianchosaint is a **standalone iteration** of the
broader Cianfhoghlaim project — operating independently of
Cianfhoghlaim at runtime — by documenting the 13 Requirements that
govern the wholesale-copy + rename + refactor of the relevant
Cianfhoghlaim assets (DLT sources, BAML schemas, CocoIndex flows,
Google ADK agents, the firecrawl_mcp browser tool client, the web
app framework, the IaC compose stacks, the agent skills, the CCC
indexing setup).

This spec is the wholesale-copy counterpart to:
- `cianchosaint-pipeline` — the data pipeline umbrella
- `cianchosaint-agentic-interaction` — the agentic interaction layer
- `cianchosaint-self-hosted-citizen` — self-hosted citizen deployment
- `cianchosaint-per-constituency-agents` — per-constituency agents

## Background

Cianchosaint was founded as a new iteration of the Cianfhoghlaim
project, but with the **defence / policing / intelligence-oversight
domain** instead of the education / long-distance learning domain. The
foundational decisions (per the `cianchosaint-repo-foundation-v1`
change + the `cianchosaint-agentic-interaction-v1` change):

- BUSL-1.1 v2 licence (British-Isles-only Additional Use Grant + 3-step
  foreign-use gate + warrant-to-enforce clause)
- 4-tier `ModelProviderRouter` (Unsloth Studio → LiteLLM → MiniMax
  Token Plan → Gemini API)
- BUSL-compliant OSINT source URL allowlist
- 8-asset wholesale-migration from Cianfhoghlaim (HMGCC rolling
  window + 5 Crown Dependencies stubs + allowlist classifier +
  official_media.baml)
- Per-spec AGENTS.md convention
- Cross-repo openspec sync documentation
- BIPP v1 / BIDP v1 / BIIP v1 flagship sub-pipelines
- Per-constituency agentic interaction layer
- Self-hosted citizen Docker bundle

The bootstrap-v2 **extends** these foundational decisions with the
**physical code wholesale-copy** that brings the spec contracts to
life. The bootstrap-v2 wholesale-copies ~15,000 LOC across 7 layers
(data platform + agents + web + IaC + skills + CCC + new combined
templates) and adds 13 Requirements that govern the wholesale-copy
contract.
...
- [`../cianchosaint-agentic-interaction/spec.md`](../cianchosaint-agentic-interaction/spec.md) — the agentic interaction layer
- [`../cianchosaint-self-hosted-citizen/spec.md`](../cianchosaint-self-hosted-citizen/spec.md) — self-hosted citizen
