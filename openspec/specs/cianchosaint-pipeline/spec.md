# cianchosaint-pipeline Capability

## Purpose

`cianchosaint-pipeline` is the umbrella capability for the `cianchosaint`
repo (the British Isles defence / policing / intelligence-oversight
open-source data platform). It covers the end-to-end stack from DLT
ingestion of public official-government sources through BAML
extraction, CocoIndex v1 embedding, LanceDB + DuckLake + MotherDuck
...
The licence in `LICENSE.md` is the load-bearing architectural
constraint.

## Background

Cianchosaint is a sibling repo to `cianfhoghlaim/cianfhoghlaim` (the
education / long-distance learning platform). It reuses 6 partial
pipelines wholesale from Cianfhoghlaim (the HMGCC rolling-window stub,
the per-Crown-Dependency stubs, and the allowlist classifier) plus the
openspec workflow, the 5 dispatchable opencode subagents, the Infisical
`dev-baile` vault, and the Lakehouse / MotherDuck stack.

Cianchosaint diverges from Cianfhoghlaim on:

- **Domain** — defence / policing / intelligence-oversight (vs education)
- **Licence** — tighter British-Isles-only OSINT grant with warrant-to-enforce (vs broader cultural grant)
- **Provider chain** — Unsloth Studio primary + 3-tier fallback (vs LiteLLM-primary)
- **Persona surfaces** — government analysts (vs students + teachers + parents)
